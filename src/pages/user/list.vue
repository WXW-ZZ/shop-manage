<template>
    <el-card shadow="never" class="border-0">
        ...
        <el-table :data="tableData" stripe style="width: 100%" v-loading="loading">
            ...
            <el-table-column prop="create_time" label="会员等级" align="center">
                <template #default="{row}">
                    {{ row.user_level?.name||"-" }}
                </template>
            </el-table-column>
            <el-table-column prop="create_time" label="登录注册" align="center">
                <template #default="{row}">
        ...
        <FormDrawer ref="formDrawerRef" :title="drawerTitle" @submit="handleSubmit">
            <el-form :model="form" ref="formRef" :rules="rules" label-width="80px" :inline="false">
                ...
                <el-form-item label="会员等级" prop="user_level_id">
                    <el-select v-model="form.user_level_id" placeholder="选择所属会员等级" style="width: 50%;">
                        <el-option v-for="item in user_level" 
                        :key="item.id" 
                        :label="item.name" 
                        :value="item.id">
