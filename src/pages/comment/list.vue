            ...
            <el-table-column type="expand">
                <template #default="{row}">
                    <div class="flex pl-18">
                        <el-avatar :size="50" shape="circle" :src="row.user.avater" fit="fill" class="mr-3"></el-avatar>
                        <div class="flex-1">
                            <h6 class="flex items-center">
                                {{ row.user.nickname ||row.user.username}}
                                <small class="text-gray-400 ml-2">{{ row.review_time }}</small>
                                <button size="small" class="ml-auto"  @click="openTextarea(row)" v-if="!row.textareaEdit&&!row.extra">回复</button>
                            </h6>
                            {{ row.review.data }}
                            <div class="py-2">
                            ...
                            <div v-if="row.textareaEdit==true">
                            ...
                            <template v-else>
                                ...
                                <h6 class="flex font-bold">客服
                                    <el-button type="info" size="small" class="ml-auto" @click="openTextarea(row,data)">修改</el-button>
                                </h6>
                                <p>{{item.data}}</p>
                                 </div>
                            </template>
...
const{
    ...
    getList:getGoodsCommentList,onGetListSuccess:(res)=>{
